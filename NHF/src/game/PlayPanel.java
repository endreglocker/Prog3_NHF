...
public class PlayPanel extends JPanel {
    ...
    public void paintComponent(Graphics graphics) {
        ...
        for (Thing thing : game.getThingList()) {
            ...
            // I could not solve to "continuously" appear from one side to another
            // solution:
            // 1st: the "thing" fully disappear
            // 2nd: "re-appear" as a whole
        ...
        if (game.isGameOver()) {
            drawText("Game Over", g2d, 0);
            drawText(playerName + " scored: " + game.getScore(), g2d, 50);
            try {
    ...
    private void drawText(String text, Graphics2D g2d, int offset) {
        ...
        //float y = (float) (worldSize / 2) + offset;

        // centering on the X axis based on the world size and the length of the text
        //float halfTextLength = g2d.getFontMetrics().stringWidth(text) / 2;
        //float x = (float) (worldSize / 2);

        g2d.setFont(title);
        g2d.drawString(text, 5, 50 + offset);
    }
